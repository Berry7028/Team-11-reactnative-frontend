...
export function MoodSelector({ selectedMoodId, onSelectMood, accentColor = '#A8DF8E' }: MoodSelectorProps) {
  return (
    <View style={{ paddingHorizontal: 24, gap: 12 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <View style={{ width: 4, height: 20, borderRadius: 999, backgroundColor: accentColor }} />
        <Text
          selectable
...
interface BodyStateSelectorProps {
  selectedBodyStateId: string | null;
  onSelectBodyState: (id: string) => void;
  accentColor?: string;
}

export function BodyStateSelector({ selectedBodyStateId, onSelectBodyState, accentColor = '#A8DF8E' }: BodyStateSelectorProps) {
  return (
    <View style={{ paddingHorizontal: 24, gap: 12 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <View style={{ width: 4, height: 20, borderRadius: 999, backgroundColor: accentColor }} />
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
        {BODY_STATES.map((state, index) => (
          <Pressable
            key={state.id}
            onPress={() => onSelectBodyState(state.id)}
            style={{
              flexBasis: '48%',
              minWidth: '48%',
              height: 56,
...
export function FreeInputSection({
  value,
  onChangeText,
  placeholder = '今の気持ちを自由に書いてね...',
  maxLength = 500,
  accentColor = '#A8DF8E',
}: FreeInputSectionProps) {
  return (
    <View style={{ paddingHorizontal: 24, gap: 12 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
